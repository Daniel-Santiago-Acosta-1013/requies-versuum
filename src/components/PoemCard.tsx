...
export const PoemCard = ({ poem }: PoemCardProps) => {
  return (
    <Link to={`/poem/${poem.id}`} className="block group">
      <article className="h-full p-6 rounded-lg shadow-md transition-all duration-300 border border-gray-100/50 dark:border-github-muted/20 hover:border-github-accent/30 dark:hover:border-github-accent-dark/30 relative overflow-hidden bg-[#f9f3e6] dark:bg-[#1a1a1a]">
        ...
        <div className="relative">
          ...
          <h3 className="text-xl font-serif mb-3 text-[#4a3c2d] dark:text-github-default-dark group-hover:text-github-accent dark:group-hover:text-github-accent-dark transition-colors">
            {poem.title}
          </h3>
          ...
          <div className="flex flex-wrap gap-2">
            {poem.tags.map(tag => (
              <span key={tag} className="inline-flex items-center text-sm text-[#8b7355] dark:text-github-subtle-dark hover:text-github-accent dark:hover:text-github-accent-dark">
